...
private
   ...
   type Layer (Num : Natural := 0; R : Activate_Pack.Routine_Acc := null; Post_P : Layer_Func_Pack.Pre_Post_ProcType := null) is limited new Abstract_Layer (Num => Num)
     with record
      Func      : Activate_Pack.Activate_Type := (if R not in null then R.Get_Activate else null);
      Deriv     : Activate_Pack.Derivative_Type := (if R not in null then R.Get_Derivative else null);
      Func_Enum : Activate_Pack.Function_Enum := (if R not in null then R.Get_Activate_Enum else Activate_Pack.None);
