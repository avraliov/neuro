...
package body Net is
   ...
   procedure Make (This : in out Net; Layers : in Layers_Acc_Array) is
      ...
      task type  Tensor_Randomizer is
         ...
         --This entry generate Gaussian distributed init values
         --in mean 0.5 and deviation 0.5
         entry Init (Item : in out Biases_Arr) ;
      ...
      begin
         ...
         loop
            ...
            or
               accept Init (Item : in out Biases_Arr) do
                  Item := (others => Get (Mean => 0.0, Deviation => 0.5));
               end Init;
            or 
               terminate;
            end select;
      ...
      begin
         for I of This.Biases loop
            declare
            begin
               I := new Biases_Arr (1 .. Layers (Idx + 1).Num);
               I.all := (others => 0.5);
               ----------------------------
               --make initialization here
               --of random values
               ----------------------------
--               Rand.Init(Item => I.all);
               Idx := Idx + 1;
            end;
         end loop;
...
--   end Finalize;
   ...
   function Back_Propogate (This : in out Net; Target : in Target_Vector; Lr : in Value_Type) return Error_Type is
      use type Layer_Package.Matrix_Pack.Real_Vector;
      Errors_Out : Layer_Package.Matrix_Pack.Real_Vector (1 .. This.Deltas (This.Deltas'Last)'Length) := Target - This.Values (This.Values'Last).all;
   begin
      declare-- deltas last layer
      begin
	 for I in This.Deltas (This.Deltas'Last).all'Range loop
--	    This.Deltas (This.Deltas'Last).all(I) := This.Derivatives(This.derivatives'Last) (This.Values(This.Values'Last)(I)) * Errors_Out(I);
	    This.Deltas (This.Deltas'Last).all(I) := Errors_Out(I); --cross-entropy
	 end loop;
      end;
      
      for I in reverse 2 .. This.Deltas'Last - 1 loop----------------------
	 declare-- errors hidden layers
            W_T : Layer_Package.Matrix_Pack.Real_Matrix := Layer_Package.Matrix_Pack.Transpose (This.Waights (I + 1).all);
            Errors : Layer_Package.Matrix_Pack.Real_Vector := W_T * This.Deltas (I + 1).all;
	 begin
	 ...
	 for I in This.Waights(W)'Range(1) loop
	    for J in This.Waights(W)'Range(2) loop
               declare
                  W_D : Value_Type := Value_Type(Lr) * This.Deltas (W) (I) * This.Values (W - 1) (J);
               begin
                  This.Waights (W) (I, J) := This.Waights (W) (I, J) + W_D;
                  null;
               end;
	    end loop;
	 end loop;
      ...
      begin
         ...
         for I in Res_Net.Activates_Enum'Range loop
            case Res_Net.Activates_Enum (I) is
               ...
               when Layer_Package.Activate_Pack.Soft => 
